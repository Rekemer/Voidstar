...
float tex_mip_level(vec2 coord, vec2 tex_size)
{
   ...
   //miplevel = log2(1.0f / (128.f * dFdx(coord.x)));
   miplevel = clamp(miplevel,0,maxMipLevel); 
   miplevel = maxMipLevel  - miplevel;
   return miplevel;
}
...
vec2 screenRes = vec2(1440,810);
vec2 feedbackRes =screenRes/70;
vec2 downscale = feedbackRes /screenRes;
void main() 
{
