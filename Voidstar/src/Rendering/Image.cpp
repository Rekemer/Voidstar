...
namespace Voidstar
{
	...
	*/
	...
	void Image::GenerateMipmaps(VkImage image, VkFormat format,
		int32_t texW, int32_t texH, uint32_t mipLevels)
	{
		// (Optionally check linear blit support as you do)

		auto cb = CommandBuffer::CreateBuffer(m_CommandPool, vk::CommandBufferLevel::ePrimary);
		cb.BeginTransfering();

		auto barrier = VkImageMemoryBarrier{
			VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, nullptr,
			0, 0, // src/dst access set below
			VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_UNDEFINED, // old/new set below
			VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED,
			image,
			{ VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, VK_REMAINING_ARRAY_LAYERS } // levelCount=1 per-step
		};

		int32_t mipW = texW;
		int32_t mipH = texH;

		// For each next level i, we: (i-1) DST->SRC, (i) UNDEFINED->DST, blit (i-1)->(i), (i-1) SRC->SRO
		for (uint32_t i = 1; i < mipLevels; ++i) {
			// (A) previous level becomes SRC for blit
			...
			// (B) destination level becomes DST for blit
			barrier.subresourceRange.baseMipLevel = i;
			barrier.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;               // or current if you track it
			...
			// (D) previous level becomes SAMPLED (we're done with it)
			barrier.subresourceRange.baseMipLevel = i - 1;
		...
		// Final level (last i) from DST -> SRO
		...
		cb.EndTransfering();
		cb.SubmitSingle();   // Prefer: submit + **signal semaphore**; wait on it in graphics submit
		cb.Free();
	}
}